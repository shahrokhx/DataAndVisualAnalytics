...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        lower_bound = 0;
        upper_bound = XX.shape[0]
        random_set = np.random.randint (lower_bound, upper_bound, size=n)
        # print np.array(random_set).shape

        samples = XX[random_set, :-1] # sampled dataset
        labels  = XX[random_set,  -1] # class labels for the sampled records
    ...
    def fitting(self):
        ...
        # pass 
        row, col = self.bootstraps_datasets[0].shape
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                # pass
                for i in range (self.num_trees):
                    votes.append(self.decision_trees[i].classify(record))
            else:
